...
interface GroupMemberRepository : JpaRepository<GroupMember, Long> {
    ...
    @Query(
        "SELECT m.group.id AS groupId, COUNT(m) AS cnt FROM GroupMember m " +
            "WHERE m.group.id IN :groupIds AND m.status = 'ACCEPTED' " +
            "GROUP BY m.group.id"
    )
    fun countAcceptedMembersByGroupIds(groupIds: List<Long>): List<AcceptedMemberCount>
