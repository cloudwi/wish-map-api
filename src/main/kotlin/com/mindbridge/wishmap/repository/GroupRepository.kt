...
interface GroupRepository : JpaRepository<Group, Long> {
    @Query("SELECT g FROM Group g JOIN g.members m WHERE m.user = :user ORDER BY g.name")
    fun findAllByMember(user: User): List<Group>
}
...
interface GroupMemberRepository : JpaRepository<GroupMember, Long> {
    ...
    @Query("SELECT m.user.id FROM GroupMember m WHERE m.group.id = :groupId")
    fun findUserIdsByGroupId(groupId: Long): List<Long>
}
