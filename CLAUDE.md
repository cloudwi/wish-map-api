...
│   ├── restaurant/# Restaurant, Visit, Like, LikeGroup, RestaurantImage
...
### 맛집
- `GET /api/v1/restaurants` - 맛집 목록 (지도 bounds 기반, APPROVED만)
- `GET /api/v1/restaurants/{id}` - 맛집 상세
- `POST /api/v1/restaurants` - 맛집 등록
- `POST /api/v1/restaurants/quick-visit` - 빠른 방문인증 (미등록 장소 자동 등록 + 방문 + 리뷰)
- `POST /api/v1/restaurants/{id}/visit` - 방문 인증
- `GET /api/v1/restaurants/place-stats` - 장소 통계 (방문수, 리뷰)
...
- `GET /api/v1/groups/{id}/restaurants` - 그룹 맛집 (구성원 방문/제보 필터)
...
- `POST /api/v1/restaurants/{id}/comments` - 댓글 작성
...
- `restaurants` - 맛집 (name, lat, lng, naverPlaceId, status, suggestedBy)
- `visits` - 방문 기록 (restaurant, user, rating)
- `comments` - 댓글
...
### Flyway 마이그레이션
- V1~V7: 초기 스키마 ~ 별점 추가
- 로컬에서는 ddl-auto=update 사용 (Flyway 비활성)
...
## 핵심 비즈니스 로직
### quickVisit (방문 인증)
1. naverPlaceId로 기존 맛집 검색, 없으면 자동 등록 (PENDING)
2. GPS 거리 체크 (100m 이내)
3. 당일 중복 방문 체크
4. Visit 저장 + 선택적 Comment 저장
